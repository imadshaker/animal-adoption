...
class Database:
    ...
    def get_random_animaux(self):
        cursor = self.get_connection().cursor()
        query = "SELECT * FROM animaux ORDER BY RANDOM() LIMIT 5"
        cursor.execute(query)
        random_animaux = cursor.fetchall()
        return [_build_animal(animal) for animal in random_animaux]


    def rechercher_animaux(self, terme_de_recherche):
        if terme_de_recherche is None:

            return []

        # Vérifier que le terme de recherche est une chaîne de caractères valide
        if not isinstance(terme_de_recherche, str):
            raise ValueError("Le terme de recherche doit être une chaîne de caractères")

        cursor = self.get_connection().cursor()
        query = ("SELECT * FROM animaux WHERE nom LIKE ? OR espece LIKE ?")
        cursor.execute(query, ('%' + terme_de_recherche + '%', '%' + terme_de_recherche + '%'))
        result_set = cursor.fetchall()
        return [_build_animal(animal) for animal in result_set]
